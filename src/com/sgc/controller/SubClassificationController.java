...
/**
 * Servlet implementation class SubClassificationController
 */
...
public class SubClassificationController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
            String subClassName = request.getParameter("subClassificationName");
            String mainClassName = request.getParameter("mainClassificationName");

            SubClassification subClass = new SubClassification();
            subClass.setSubClass(subClassName);
            subClass.setMainClass(mainClassName);

            int status = SubClassificationDAO.AddSubClassification(subClass);
            if (status > 0) {
